...
export default class Journal {
    ...
    createLinkForCurrentDay() {
        const date = new Date();
        const todayFilename = `${date.getFullYear()}-${("0" + date.getMonth()).slice(-2)}-${("0" + date.getDay()).slice(-2)}.md`;
    ...
    createNewEntry(nextDay = false) {
        ...
        let filename: string;
        if (nextDay) {
            filename = `${date.getFullYear()}-${("0" + date.getMonth()).slice(-2)}-${("0" + date.getDay()).slice(-2)}.md`;
        } else {
            const dateTomorrow = new Date();
            dateTomorrow.setDate(date.getDate() + 1);
            filename = `${dateTomorrow.getFullYear()}-${("0" + dateTomorrow.getMonth()).slice(-2)}-${("0" + dateTomorrow.getDay()).slice(-2)}.md`;
        }
