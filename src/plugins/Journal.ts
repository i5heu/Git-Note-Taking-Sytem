...
export default class Journal {
    ...
    createLinkForCurrentDay() {
        const date = new Date();
        const todayFilename = `${date.getFullYear()}-${("0" + (date.getMonth() + 1)).slice(-2)}-${("0" + date.getDate()).slice(-2)}.md`;
    ...
    createNewEntry(nextDay = false) {
        ...
        let filename: string;
        if (nextDay) {
            filename = `${date.getFullYear()}-${("0" + (date.getMonth() + 1)).slice(-2)}-${("0" + date.getDate()).slice(-2)}.md`;
        } else {
